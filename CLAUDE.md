...
- **TTS**: `lib/speak.ts` — 3 providers (`avatarStore.provider`): **Piper** (offline neural, Bahasa Indonesia), **Edge neural** (free, `msedge-tts`, no key, needs internet), **macOS `say`** (offline). `synth()` falls back piper→edge→local. All via `commands/avatar.rs`. Voice by gender (woman/man) × language (`voiceLang`: id/en/multi). Lip-sync (`lib/lipsync.ts`) drives the `Aa` viseme from audio RMS; mouth follows `lipSync.speaking` (works during Claude streaming too); emotion blendshape lowered while talking so the viseme stays visible.
  - **Piper** (`synthesize_piper`/`piper_status`/`piper_download`): downloaded once on first use to app-data `piper/` (button in `AvatarVoiceSettings`, ~105 MB) — NOT bundled, so CI/build is untouched. Needs BOTH the `rhasspy/piper` binary tarball (`2023.11.14-2`, ships exe + `espeak-ng-data`) AND the matching `rhasspy/piper-phonemize` tarball (`2023.11.14-4`, ships the `libespeak-ng`/`libpiper_phonemize`/`libonnxruntime` dylibs the binary tarball omits). Run = binary with `DYLD/LD_LIBRARY_PATH = piper-phonemize/lib:piper`, `--espeak_data piper/espeak-ng-data`, text on stdin → WAV 16-bit mono 22.05kHz. Model: `id_ID-news_tts-medium` (HF rhasspy/piper-voices).
